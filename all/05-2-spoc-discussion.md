...
3. ucore的进程控制块数据结构是如何组织的？主要字段分别表示什么？
   ...
   分为四部分：进程标识信息，进程状态信息，进程占用的资源，保护现场所用的信息，描述进程所在队列的信息。

   主要字段例如：name，pid，runs，parent等
...
2. 第一个用户进程执行的代码在哪里？它是什么时候加载到内存中的？
   ...
   init_main()，在硬盘里，proc_init()执行的时候加载到内存中
...
3. 什么是僵尸进程和孤儿进程？
   ...
   僵尸进程：子进程退出后，父进程未获取子进程状态信息，子进程的进程描述符仍然保存在系统中

...
1. x86/RV中断处理中硬件压栈内容？用户态中断和内核态中断的硬件压栈有什么不同？
   1. 当前程序的EIP，CS，EFLAG，Error code，ss和esp
   2. 用户态中断特权级会变，内核态不会
2. 为什么在用户态的中断响应要使用内核堆栈？
   1. 避免内核堆栈中的数据（如OS代码）被用户修改导致严重后果
3. x86中trap类型的中断门与interrupt类型的中断门有啥设置上的差别？如果在设置中断门上不做区分，会有什么可能的后果?
   1. Trap处理过程中中断门还开着，interrupt中则关闭
   2. 不区分的话可能导致陷入无尽的中断，以及trap处理时不能响应该响应的中断

### 4.3 练习四和五 ucore内核映像加载和函数调用栈分析

1. ucore中，在kdebug.c文件中用到的函数`read_ebp`是内联的，而函数`read_eip`不是内联的。为什么要设计成这样？
   1. 如果调用函数，ebp值将被改变成为新的ebp，难以获取；而eip则被压到栈里，从栈里拿就行

### 4.4 练习六 完善中断初始化和处理

1. CPU加电初始化后中断是使能的吗？为什么？
   1. 不是，因为刚加电完之后，还没加载中断处理程序等代码
